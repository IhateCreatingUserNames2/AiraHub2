...
MONGODB_URL = os.getenv("MONGODB_URL", "mongodb+srv://username:password@cluster.mongodb.net/aira_hub?retryWrites=true&w=majority")
MONGODB_DB_NAME = os.getenv("MONGODB_DB_NAME", "aira_hub")
...
class MongoDBStorage:
    ...
    async def get_agent_by_tool(self, tool_name: str) -> Optional[AgentRegistration]:
        ...
        agent_dict = await self.db.agents.find_one({"mcp_tools.name": tool_name})
        if agent_dict:
            # Update cache
    ...
    async def list_tools(self) -> List[Dict[str, Any]]:
        ...
        tools = []
        cursor = self.db.agents.find({}, {"mcp_tools": 1, "name": 1, "agent_id": 1})
    ...
    async def delete_agent(self, agent_id: str):
        ...
        if agent:
            ...
            await self.db.agents.delete_one({"agent_id": agent_id})
...
storage = MongoDBStorage(MONGODB_URL, MONGODB_DB_NAME)
