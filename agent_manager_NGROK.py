...
async def register_with_aira_hub(
    ...
    if connection_type == "stdio":
         ...
         base_proxy_url = AGENT_MANAGER_PUBLIC_URL  # Use the public URL variable
         if not os.getenv("AGENT_MANAGER_PUBLIC_URL"):  # Check env var directly for warning
             logger.warning(
                 "AGENT_MANAGER_PUBLIC_URL not set. Registering local proxy URL, which may not be reachable by a remote AIRA Hub.")

         proxy_url = f"{base_proxy_url.rstrip('/')}/proxy/{server_config.id}"
         registration_payload["mcp_stream_url"] = proxy_url
         registration_payload["mcp_url"] = proxy_url
         registration_payload["stdio_command"] = server_config.connection.command
         logger.info(f"Registering stdio server {server_config.name} with *public* proxy URL: {proxy_url}")
...
if __name__ == "__main__":
    import uvicorn
    # Log the BIND address here
    logger.info(f"Starting Agent Manager server, binding to {AGENT_MANAGER_BIND_HOST}:{AGENT_MANAGER_PORT}")
    uvicorn.run(
        "__main__:app",
        host=AGENT_MANAGER_BIND_HOST, # <-- Use the BIND host
